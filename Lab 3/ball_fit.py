...
def fit_hill_climbing():
    ...
    def neighbors(theta):
        ...
        while cont <num_neighbors:
            neighbors_list.append(np.array([theta[0]+delta*np.cos(cont*alpha),theta[1]+ delta*np.sin(cont*alpha)]))
            cont +=1
...
def fit_simulated_annealing():
    ...
    def random_neighbor(theta):
        ...
        return np.array([theta[0] +delta*np.cos(alpha),theta[1]+delta*np.sin(alpha)])
